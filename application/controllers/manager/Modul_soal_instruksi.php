...
class Modul_soal_instruksi extends Member_Controller {
	private $kode_menu = 'modul-soal-instruksi';
	private $kelompok = 'modul';
	private $url = 'manager/modul_soal_instruksi';
    ...
    public function index($id_soal=null){
        ...
        $this->template->display_admin($this->kelompok.'/modul_soal_instruksi_view', 'Mengelola Soal', $data);
    }
    ...
    function tambah(){
        ...
        if($this->form_validation->run() == TRUE){
			...
			$subtest = $this->input->post('tambah-subtes', FALSE);
			...
			$sqlCount = "
				SELECT  COUNT(`instruksi_subtes`) as hasil
				FROM    cbt_subtest_instruksi
				WHERE   cbt_subtest_instruksi.instruksi_topik_id = ".$id_topik."
				AND   	cbt_subtest_instruksi.instruksi_subtes = ".$subtest."
			";
			...
			// else if ($topicount >= 1){
        	// 	$status['status'] = 0;
            // 	$status['pesan'] = 'Instruksi sudah tersedia';
			// }
			else{
        		$data['instruksi_topik_id'] = $id_topik;				
        		$data['instruksi_text'] = $soal;
	        	$data['instruksi_subtes'] = $subtest;

	        	$upload = 0;

	        	if(!empty($id_soal)){
	        		$this->cbt_soal_model->updateSubtes('instruksi_id', $id_soal, $data);
	        	}else{
	        		$this->cbt_soal_model->saveSubtes($data);	
	        	}
    ...
    function get_by_id($id=null){
    	$data['data'] = 0;
		if(!empty($id)){
			$query = $this->cbt_soal_model->get_by_kolom_subtes('instruksi_id', $id);
			if($query->num_rows()>0){
				$query = $query->row();
				$data['data'] = 1;
				$data['id'] = $query->instruksi_id;
				$soal = $query->instruksi_text;
				$soal = str_replace("[base_url]", base_url(), $soal);
				$data['soal'] = $soal;
				$data['subtes'] = $query->instruksi_subtes;
				$data['id_topik'] = $query->instruksi_topik_id;
			}
		}
    ...
    function get_datatable(){
		...
		$query = $this->cbt_soal_model->get_datatable_subtest($start, $rows, 'instruksi_text', $search, $topik);
		$iFilteredTotal = $query->num_rows();
		
		$iTotal= $this->cbt_soal_model->get_datatable_subtest_count('instruksi_text', $search, $topik)->row()->hasil;
	    ...
	    foreach ($query as $temp) {			
			$record = array();
			// $record[] =  str_replace("[base_url]", base_url(), $soal_nomor);;

			$instruksi_id = $temp->instruksi_id;
			$instruksi_topik_id = $temp->instruksi_topik_id;
			$instruksi_text = $temp->instruksi_text;	
			$instruksi_subtes = $temp->instruksi_subtes;

			// $soal_nomors = $temp->soal_nomor;
			// $record[] = $soal_nomors;
			$record[] = ++$x;

			$mysqli = new mysqli("localhost","root", "","celestia_dbmtfpsikotes");
			$sqlTopik = "
			SELECT  cbt_topik.topik_nama as topik_nama
			FROM    cbt_topik
			WHERE   cbt_topik.topik_id = ".$instruksi_topik_id." ";
			$topikNama = '';
			if($result = mysqli_query($mysqli, $sqlTopik)){
				while($row = mysqli_fetch_array($result)){
	
					$topikNama =$row['topik_nama'];
				}
			}

			$record[] = $topikNama;
			$record[] = $instruksi_subtes;

			// $record[] = ++$i;
			// $soal = $temp->soal_detail;
			$instruksi_text = str_replace("[base_url]", base_url(), $instruksi_text);
            $record[] = $instruksi_text;
	        ...
	        //     	<a onclick="jawaban(\''.$instruksi_id.'\')" title="Tambah Jawaban" style="cursor: pointer;"><span class="glyphicon glyphicon-question-sign"></span></a>
	        //     	<a onclick="edit(\''.$instruksi_id.'\')" title="Edit Soal" style="cursor: pointer;"><span class="glyphicon glyphicon-edit"></span></a>
	        //     	<a onclick="hapus(\''.$instruksi_id.'\')" title="Hapus Soal" style="cursor: pointer;"><span class="glyphicon glyphicon-remove"></span></a>
	        //     	</div>
	        //     ';
            // }else{
            	$record[] = '<div style="text-align: center;">
