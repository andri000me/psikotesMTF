...
class Tes_subtes extends Tes_Controller {
    ...
    public function index($tesuser_subtes,$tesuser_id){
		echo json_encode($tesuser_subtes);
		echo json_encode($tesuser_id);
	...
	function konfirmasi_test($tes_id=null){
    	if(!empty($tes_id)){
    		...
    		if($query_tes->num_rows()>0){
				...
				if($result = mysqli_query($mysqli, $sqlInstruksis)){
					while($row = mysqli_fetch_array($result)){
						$tanggal_tes = new DateTime($row['end_time']);
					}
				}

				// $tanggalsss = new DateTime($tanggal_tes);
				// $tanggal = new DateTime();

				// $tanggal_tes = new DateTime($query_tes->tes_end_time);
				$tanggal_tes = new DateTime($query_tes->tes_end_time);
    			if($tanggal<$tanggal_tes){ // error
    				...
    				if($this->cbt_tes_user_model->count_by_user_tes($user_id, $query_tes->tes_id)->row()->hasil==0){
						...
						WHERE	cbt_subtest_instruksi.instruksi_topik_id = ".$user_id."
						AND		cbt_subtest_instruksi.instruksi_subtes = ".$user_id."
						";
				        ...
				        if($data['tes_max_score']>0){
				        	$this->template->display_tes($this->kelompok.'/tes_subtest_view', 'Mulai Tes', $data);	
				        }else{
